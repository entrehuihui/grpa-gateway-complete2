...
func loadConfig() config {
	return config{
		CertPemPath: viper.GetString("Certs.serverDir") + "server.pem",
		CertKeyPath: viper.GetString("Certs.serverDir") + "server.key",
		grpcPort:    ":" + strconv.Itoa(viper.GetInt("Server.port")),
		serverName:  viper.GetString("Server.serviceAddr"),
	}
}
...
func startGW(cfg config) *http.ServeMux {
	...
	gwmux.Handle("/"+viper.GetString("Server.port")+"/", mux)
